...
function __construct($lista, $datos,$num_tupla,$max_tuplas,$totalTuplas,$num_pagina, $orden, $origen,$acciones,$trabajos,$num_trabajos, $trabajosNota, $alumnos, $notas, $datosSearch){
...
?>
     <section class="pagina"  style="min-height: 500px; height: 100%;">
                ...
                <tr>
                    ...
                    <th><?php echo $strings["Nota"]; ?></th>
                    <th><?php echo $strings['Nota Total']; ?></th> 
                    <td>
                     <?php 
...
<?php       
            //Mientras el numero de tuplas no llegue al máximo y haya tuplas en la BD
            while($row_search = mysqli_fetch_array($this->datosSearch)) {
                
?>          
                    <tr>
                    <td><?php echo $row_search['login']; ?></td>
                    <td><?php echo $row_search['Nombre']; ?></td>
                    <td><?php echo $row_search['IdTrabajo']; ?></td>
                    <td><?php echo $row_search['NombreTrabajo']; ?></td>
                    <td><?php echo $row_search['NotaTrabajo']; ?></td>
                    <td><?php echo $this->notas[$row_search['login']]; ?></td>
                    ...
                    foreach ($this->acciones as $key => $value) {
                           ...
                           if($value == 'DELETE'){
                            ?>
                    <a href="../Controllers/NOTA_TRABAJO_Controller.php?action=DELETE&login=<?php echo $row_search["login"]?>&IdTrabajo=<?php echo $row_search["IdTrabajo"]?>"><input type="image" src="../Views/images/delete.png" name="action" title="<?php echo $strings['Eliminar'] ?>" value="DELETE"></a>
